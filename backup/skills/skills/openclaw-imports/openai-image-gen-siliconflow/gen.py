...
def get_model_defaults(model: str) -> tuple[str, str]:
    ...
    if model == "Qwen/Qwen-Image":
        # SiliconFlow Qwen model: quality not supported
        return ("1024x1024", "")
    elif model == "dall-e-2":
...
def request_images(
    ...
    # SiliconFlow uses image_size, OpenAI uses size
    if model == "Qwen/Qwen-Image":
        args["image_size"] = size
    else:
        args["size"] = size

    # Quality parameter - dall-e-2 and Qwen don't accept this parameter
    if model not in ("dall-e-2", "Qwen/Qwen-Image") and quality:
