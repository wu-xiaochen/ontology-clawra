...
### Cron job delivery failure
If a cron job runs but you see `"delivery_error": "no delivery target resolved for deliver=origin"`, the `origin` target doesn't resolve in your setup. Fix: update the job's `deliver` field to use a specific platform instead:
- Telegram: `telegram:CHAT_ID` (e.g., `telegram:8578764073`)
- Discord: `discord:#channel-name`
- WhatsApp: check pairing directory for the correct ID

Never assume `origin` will work — always verify or use a specific platform target.
